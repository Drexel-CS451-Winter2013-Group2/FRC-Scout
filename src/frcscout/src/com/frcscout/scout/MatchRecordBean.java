...
public class MatchRecordBean {
    ...
    public void updateMatchRecord() {
    	if (id != null) {
	        ...
	        String q = "UPDATE match_record_2013 SET " +
	            ...
	            "teleop_pyramid = ?, pyramid_level = ?, play_style = ?, " +
	            "confidence = ?, ability = ?, fouls = ?, " +
	            "technical_fouls = ?, comments = ?, path = ? WHERE id = ?;";
	        ...
	        try {
	            ...
	            e.printStackTrace();
	        } finally {System.out.println("Unable to update event because id is null");
	            try {
	                conn.close();
	                st.close();
	            } catch (SQLException e) {
    ...
    public String loadMatchRecords() {
        PreparedStatement st = null;
        ResultSet rs = null;
        ...
        try {
            ...
            while (rs.next()) {
                ...
                o.put("match_id", rs.getInt("match_id"));
